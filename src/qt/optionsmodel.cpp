...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::SND);
    ...
    if (!settings.contains("nAnonymizeSnodecoinAmount"))
        settings.setValue("nAnonymizeSnodecoinAmount", 1000);

    nAnonymizeSnodecoinAmount = settings.value("nAnonymizeSnodecoinAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeSnodecoinAmount"))
        SoftSetArg("-anonymizesnodecoinamount", settings.value("nAnonymizeSnodecoinAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in snodecoin.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeSnodecoinAmount:
            return QVariant(nAnonymizeSnodecoinAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeSnodecoinAmount:
            nAnonymizeSnodecoinAmount = value.toInt();
            settings.setValue("nAnonymizeSnodecoinAmount", nAnonymizeSnodecoinAmount);
            emit anonymizeSnodecoinAmountChanged(nAnonymizeSnodecoinAmount);
            break;
        case CoinControlFeatures:
