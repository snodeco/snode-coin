...
#if defined(NDEBUG)
#error "Snodecoin cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "Snodecoin Signed Message:\n";
        ...
        if(tx.IsZerocoinSpend()){
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (!ContextualCheckCoinSpend(spend, chainActive.Tip(), txid))
                    return state.Invalid(error("%s: zSND spend in tx %s failed to pass context checks", __func__, txid.GetHex()));
            }
        } else {
            LOCK(pool.cs);
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount)
{
    ...
    if (nHeight <= Params().PRESALE_START_BLOCK()){
        ret = 0;
    } else if (nHeight <= Params().PRESALE_END_BLOCK() && nHeight >= Params().PRESALE_START_BLOCK() + 1) {
        ret = 80 * COIN;
    } else if (nHeight >= Params().PRESALE_END_BLOCK() + 1) {
        ret = blockValue * 0.8; 
    }
    return ret; 
}
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from Snodecoin
...
{
    RenameThread("snodecoin-scriptch");
    scriptcheckqueue.Thread();
}

void RecalculateZSNDMinted()
{
...
bool RecalculateSNDSupply(int nHeightStart)
{
...
bool UpdateZSNDSupply(const CBlock& block, CBlockIndex* pindex)
{
    std::list<CZerocoinMint> listMints;
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    if (Params().Zerocoin_Block_RecalculateAccumulators() >= 0 && pindex->nHeight == Params().Zerocoin_Block_RecalculateAccumulators() + 1) {
        ...
        RecalculateSNDSupply(Params().Zerocoin_StartHeight());
    }

    //Track zSND money supply in the block index
    if (!UpdateZSNDSupply(block, pindex))
        return state.DoS(100, error("%s: Failed to calculate new zSND supply for block=%s height=%d", __func__,
...
void static UpdateTip(CBlockIndex* pindexNew)
{
    ...
    // If turned on AutoZeromint will automatically convert SND to zSND
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // Snodecoin: We use certain sporks during IBD, so check to see if they are
