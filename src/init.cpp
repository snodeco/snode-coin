...
#include "config/snodecoin-config.h"
#endif
...
void PrepareShutdown()
{
    ...
    RenameThread("snodecoin-shutoff");
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "snodecoin.conf"));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "snodecoind.pid"));
#endif
    strUsage += HelpMessageOpt("-reindex", _("Rebuild block chain index from current blk000??.dat files") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexaccumulators", _("Reindex the accumulator database") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexmoneysupply", _("Reindex the SND and zSND money supply statistics") + " " + _("on startup"));
    ...
    strUsage += HelpMessageOpt("-port=<port>", strprintf(_("Listen for connections on <port> (default: %u or testnet: %u)"), 18988, 18990));
    ...
    if (GetBoolArg("-help-debug", false))
        strUsage += HelpMessageOpt("-mintxfee=<amt>", strprintf(_("Fees (in SND/Kb) smaller than this are considered zero fee for transaction creation (default: %s)"),
            FormatMoney(CWallet::minTxFee.GetFeePerK())));
    strUsage += HelpMessageOpt("-paytxfee=<amt>", strprintf(_("Fee (in SND/kB) to add to transactions you send (default: %s)"), FormatMoney(payTxFee.GetFeePerK())));
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, tor, mempool, net, proxy, http, libevent, snodecoin, (obfuscation, swifttx, masternode, mnpayments, mnbudget, zero)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-minrelaytxfee=<amt>", strprintf(_("Fees (in SND/Kb) smaller than this are considered zero fee for relaying (default: %s)"), FormatMoney(::minRelayTxFee.GetFeePerK())));
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all Snodecoin specific functionality (Masternodes, Zerocoin, SwiftTX, Budgeting) (0-1, default: %u)"), 0));
    ...
    strUsage += HelpMessageOpt("-masternodeaddr=<n>", strprintf(_("Set external address:port to get to this masternode (example: %s)"), "192.168.1.1:18988"));
...
//    strUsage += "  -anonymizesnodecoinamount=<n>     " + strprintf(_("Keep N SND anonymized (default: %u)"), 0) + "\n";
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) %i The Snodecoin Core Developers"), "2018")) + "\n" +
           "\n" + 
...
{
    RenameThread("snodecoin-loadblk");
...
/** Sanity checks
 *  Ensure that Snodecoin is running in a usable environment with all
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Snodecoin Core is shutting down."));
...
#endif
    // Make sure only a single Snodecoin process is using the data directory.
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Snodecoin Core is probably already running."), strDataDir));
    ...
    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("Snodecoin version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    while (!fLoaded) {
        ...
        do {
            try {
                ...
                //Snodecoin specific: zerocoin and spork DB's
                ...
                // Snodecoin: load previous sessions sporks if we have them.
                ...
                if (GetBoolArg("-reindexmoneysupply", false)) {
                    if (chainActive.Height() > Params().Zerocoin_StartHeight()) {
                        RecalculateZSNDMinted();
                        RecalculateZSNDSpent();
                    }
                    RecalculateSNDSupply(1);
                }
                ...
                // Snodecoin: recalculate Accumulator Checkpoints that failed to database properly
...
// ********************************************************* Step 8: load wallet
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK) {
            if (nLoadWalletRet == DB_CORRUPT)
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of Snodecoin Core") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE) {
                strErrors << _("Wallet needed to be rewritten: restart Snodecoin Core to complete") << "\n";
    ...
    // Snodecoin to be removed testing 04/08/2018
    /*
    ...
    */
    ...
    //strBudgetMode = GetArg("-budgetvotemode", "auto"); // Snode
    ...
    fEnableZeromint = false;    // Snodecoin force false
    ...
    nZeromintPercentage = 0;    // Snodecoin force zero
...
//    }
//
//    nAnonymizeSnodecoinAmount = GetArg("-anonymizesnodecoinamount", 0);
//    if (nAnonymizeSnodecoinAmount > 999999) nAnonymizeSnodecoinAmount = 999999;
//    if (nAnonymizeSnodecoinAmount < 2) nAnonymizeSnodecoinAmount = 2;
    ...
    LogPrintf("Anonymize Snodecoin Amount %d\n", nAnonymizeSnodecoinAmount);
    ...
    /* Denominations
       ...
       For example:
       1SND+1000 == (.1SND+100)*10
       10SND+10000 == (1SND+1000)*10
    */
