...
UniValue getinfo(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "  \"balance\": xxxxxxx,         (numeric) the total snodecoin balance of the wallet (excluding zerocoins)\n"
        //    "  \"zerocoinbalance\": xxxxxxx, (numeric) the total zerocoin balance of the wallet\n"
            ...
            "  \"paytxfee\": x.xxxx,         (numeric) the transaction fee set in snodecoin/kb\n"
            "  \"relayfee\": x.xxxx,         (numeric) minimum relay fee for non-free transactions in snodecoin/kb\n"
    ...
    obj.push_back(Pair("zSNDsupply", zsnoObj));
    */
#ifdef ENABLE_WALLET
    if (pwalletMain) {
...
UniValue validateaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "validateaddress \"snodecoinaddress\"\n"
            "\nReturn information about the given snodecoin address.\n"
            "\nArguments:\n"
            "1. \"snodecoinaddress\"     (string, required) The snodecoin address to validate\n"
            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,         (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"snodecoinaddress\", (string) The snodecoin address validated\n"
...
#ifdef ENABLE_WALLET
        // Case 1: Snodecoin address and we have full public key:
...
UniValue createmultisig(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 2) {
        string msg = "createmultisig nrequired [\"key\",...]\n"
                     ...
                     "2. \"keys\"       (string, required) A json array of keys which are snodecoin addresses or hex-encoded public keys\n"
                     "     [\n"
                     "       \"key\"    (string) snodecoin address or hex-encoded public key\n"
                     "       ,...\n"
                     "     ]\n"
...
UniValue verifymessage(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            "verifymessage \"snodecoinaddress\" \"signature\" \"message\"\n"
            "\nVerify a signed message\n"
            "\nArguments:\n"
            "1. \"snodecoinaddress\"  (string, required) The snodecoin address to use for the signature.\n"
